...
OneButton::OneButton(int pin, int activeLow)
{
  ...
  _clickTicks = 950;        // number of millisec that have to pass by before a click is detected.
  _pressTicks = 200;       // number of millisec that have to pass by before a lonn button press is detected.
...
// Serial.println(_state);
  ...
  if (_state == 0) { // waiting for menu pin being pressed.
    ...
    else if (now > _startTime + _clickTicks) {
