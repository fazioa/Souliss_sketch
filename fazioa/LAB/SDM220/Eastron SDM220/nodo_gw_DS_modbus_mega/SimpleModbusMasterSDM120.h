...
//MACRO FOR TURN ON OR OFF THEN PACKET_OFF
 #define modbus_packet_Off(packet) packet.connection = 0		//Packet wont be sent
 #define modbus_packet_On(packet) packet.connection = 1		//Packet will be sent in al loop
 #define modbus_packet_Once(packet) packet.connection = 2      //Packet will be sent only one time then will turn connection=0
 
 //MACRO FOR ENABLE DISABLE disable_retriesCounter
 #define modbus_enable_retriesForever(packet) packet.enable_retriesCounter=0    
 #define modbus_disable_retriesForever(packet) packet.enable_retriesCounter=1//by default
