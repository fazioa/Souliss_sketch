...
void setup()
{
  ...
  Souliss_SetT14(memory_map, SLOT_REMOTE_CONTROLLER);
  pinMode(PIN_OUTPUT_APRIPORTA, OUTPUT); //PIN FOR PULSE SLOT_APRIPORTA
  Souliss_SetT14(memory_map, SLOT_APRIPORTA);
...
void loop()
{
  ...
  else if (abs(millis() - tmr_slow) > time_base_slow)
  {
    ...
    if (!(phase_slow % 1))
    {
      Souliss_Logic_T14(memory_map, SLOT_REMOTE_CONTROLLER, &data_changed);
